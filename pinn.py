...
for i, name in enumerate(target_names):
    np.save(STAGE3_OUTPUT / f"{sanitize_filename(name)}_true.npy", Y_true[:, i])
    np.save(STAGE3_OUTPUT / f"{sanitize_filename(name)}_pred.npy", Y_pred[:, i])
...
for i, name in enumerate(target_names):
    ...
    plt.scatter(yt, yp, color='royalblue', edgecolor='k', s=80)

    # Annotate points with % error
    for j in range(len(yt)):
        pct_err = 100 * np.abs(yt[j] - yp[j]) / (yt[j] + 1e-9)
        plt.text(yt[j], yp[j], f"{pct_err:.1f}%", fontsize=7, color='darkred')

    plt.plot([yt.min(), yt.max()], [yt.min(), yt.max()], 'r--', alpha=0.7)
    plt.title(f"{name} Predictions with % Error")
    plt.xlabel("True Values")
    plt.ylabel("Predicted Values")
    plt.tight_layout()
    plt.savefig(STAGE3_OUTPUT / f"{sanitize_filename(name)}_scatter_pct.png")
    plt.close()

# -------------------------
# BAR CHART OF MAE AND SMAPE
# -------------------------
x = np.arange(len(df_results))
width = 0.35
fig, ax = plt.subplots(figsize=(14, 6))
ax.bar(x - width / 2, df_results["MAE"], width, label="MAE", color='royalblue')
ax.bar(x + width / 2, df_results["SMAPE_%"], width, label="SMAPE %", color='darkorange')
ax.set_xticks(x)
ax.set_xticklabels(df_results["Target"], rotation=45, ha='right')
ax.set_ylabel("Error Metrics")
ax.set_title("Error Metrics per Target")
ax.legend()
plt.tight_layout()
plt.savefig(STAGE3_OUTPUT / "bar_metrics.png")
plt.close()
...
print("\n✅ Done! Metrics, plots, and table saved in:", STAGE3_OUTPUT)
print(df_results)
